...
-- ****************************************************************************************
-- Populate atmp.F0101 with ship-to data from raw sage file
--
...
--  
-- ****************************************************************************************
IF EXISTS(SELECT * FROM SYS.objects WHERE TYPE = 'P' AND name = N'usp_F0101_Load_ShipTo')
	DROP PROCEDURE dbo.usp_F0101_Load_ShipTo
GO

CREATE PROCEDURE dbo.usp_F0101_Load_ShipTo
AS
BEGIN
	...
	(
		select  
			@startingRowNum
			+ ROW_NUMBER() OVER(ORDER BY UKID) 
			- 1  as xrn,
			UKID
		from #tempIntermediate
	) as y on x.UKID = y.UKID

	-- now set the associated address based on JDE lookup
	update #tempIntermediate
		set JDEAssociated = (select ABAN8 from atmp.F0101 where ABALKY = CustCode collate database_Default)
                              
	insert into atmp.F0101
	select 
		ROWNUM AS ABAN8,
		UKID COLLATE DATABASE_DEFAULT AS ABALKY, 
		...
		N'S3' COLLATE DATABASE_DEFAULT AS ABAT1, 
	...
	from #tempIntermediate
	where JDEAssociated is not null

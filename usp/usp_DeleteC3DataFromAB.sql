...
GO
-------------------------------------------------------------------------------------------
-- Delete C3 data from the temporary tables F0101, F0111, F0115, F0116
--
-- 13-Feb-2020 R.Lillback Created Initial Version
--------------------------------------------------------------------------------------------
IF EXISTS(SELECT * FROM SYS.objects WHERE TYPE = 'P' AND name = N'usp_DeleteC3DataFromAB')
	DROP PROCEDURE dbo.usp_DeleteC3DataFromAB
GO

CREATE PROCEDURE dbo.usp_DeleteC3DataFromAB
AS
BEGIN
	...
	insert into #AN8sToRemove
		select ABAN8 from atmp.F0101 where ABAT1 = N'C3'

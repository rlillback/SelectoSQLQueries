...
GO
-------------------------------------------------------------------------------------------
-- Delete V3 data from the temporary tables F0101, F0111, F0115, F0116
--
-- 11-Feb-2020 R.Lillback Created Initial Version
--------------------------------------------------------------------------------------------
IF EXISTS(SELECT * FROM SYS.objects WHERE TYPE = 'P' AND name = N'usp_DeleteV3DataFromAB')
	DROP PROCEDURE dbo.usp_DeleteV3DataFromAB
GO

CREATE PROCEDURE dbo.usp_DeleteV3DataFromAB
AS
BEGIN
	...
	insert into #AN8sToRemove
		select ABAN8 from atmp.F0101 where ABAT1 = N'V3'

...
-- ****************************************************************************************
-- Create remit who's who records to populate WWALPH et al.  Without this the AB
...
IF EXISTS(SELECT * FROM SYS.objects WHERE TYPE = 'P' AND name = N'usp_F0111_Remit_Load')
	DROP PROCEDURE dbo.usp_F0111_Remit_Load
GO

CREATE PROCEDURE dbo.usp_F0111_Remit_Load
AS
BEGIN
	SET NOCOUNT ON;
    ...
    declare whoCursor scroll cursor for
		select aban8 from atmp.F0101 where ABAT1 in (N'R3')
	open whoCursor;

...
IF EXISTS(SELECT * FROM SYS.objects WHERE TYPE = 'P' AND name = N'usp_F0115_Customer_Phones')
	DROP PROCEDURE dbo.usp_F0115_Customer_Phones
GO

CREATE PROCEDURE [dbo].[usp_F0115_Customer_Phones] 
AS
BEGIN
-- ****************************************************************************************
-- Import customer phone and fax numbers 
--
-- NOTES:
--
-- HISTORY:
...
-- ****************************************************************************************
	...
	insert into #TempPhones
		select 
			...
			(LTRIM(RTRIM(C.TelephoneNo)) + N' ' + ISNULL(LTRIM(RTRIM(C.TelephoneExt)),N'')) COLLATE Latin1_General_CI_AS_WS as WPPH1,
	...
	insert into #TempPhones
		select 
			...
			N'' COLLATE Latin1_General_CI_AS_WS as WPPAR1,
			LTRIM(RTRIM(C.FaxNo)) COLLATE Latin1_General_CI_AS_WS as WPPH1,
			@user COLLATE Latin1_General_CI_AS_WS as WPUSER,
			@pid COLLATE Latin1_General_CI_AS_WS as WPPID,
			@jToday as WPUPMJ,
			@jobn COLLATE Latin1_General_CI_AS_WS as WPJOBN,

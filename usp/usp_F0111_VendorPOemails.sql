...
-- ****************************************************************************************
-- Populate the vendor's PO email addresses (Type Code = P)
--
--
-- NOTES:
--
...
IF EXISTS(SELECT * FROM SYS.objects WHERE TYPE = 'P' AND name = N'usp_F0111_VendorPOemails') begin
	print('dropping procedure dbo.usp_F0111_VendorPOemails');
	DROP PROCEDURE dbo.usp_F0111_VendorPOemails
end
GO

print('creating procedure dbo.usp_F0111_VendorPOemails');
go

CREATE PROCEDURE dbo.usp_F0111_VendorPOemails
AS
BEGIN
	SET NOCOUNT ON;
    ...
    declare nameCursor scroll cursor for
		select alky, email from dbo.ods_VendorPOemails
	open nameCursor;
	...
	while @@FETCH_STATUS = 0 begin		
		...
		if (@who is not null) begin
			...
			values( @who, @lineNo, 0, @email, @email, N'P', @user, @pid, @jToday, @jobn, @tNow, 0, 0, 0, 0, 0 )
		end else begin
			insert into [dbo].[error_CustomerEmailLoad]
				values('Custom Error', 'Custom Error',cast((@alky + ' was not found in the address book F0101') as varchar(4000)),N'Error populating vendor in dbo.usp_F0111_VendorPOemails')
		end
